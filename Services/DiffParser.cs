...
public partial class DiffParser
{
    ...
    public static ParsedPatch Parse(string patch)
    {
        ...
        foreach (var l in lines)
        {
            ...
            var dl = l switch
            {
                var x when x.StartsWith("+") => new ParsedLine { Type = DiffLineType.Added, Content = l.Length > 1 ? l[1..] : "", OldLineNumber = null, NewLineNumber = nl++ },
                var x when x.StartsWith("-") => new ParsedLine { Type = DiffLineType.Removed, Content = l.Length > 1 ? l[1..] : "", OldLineNumber = ol++, NewLineNumber = null },
                var x when x.StartsWith(" ") || l == "" => new ParsedLine { Type = DiffLineType.Context, Content = l.Length > 1 ? l[1..] : "", OldLineNumber = ol++, NewLineNumber = nl++ },
