...
public partial class DiffParser
{
    ...
    public static ParsedPatch Parse(string patch)
    {
        ...
        foreach (var rl in patch.Split('\n'))
        {
            ...
            if (dm.Success)
            {
                cf = new() { OldPath = dm.Groups["old"].Value, NewPath = dm.Groups["new"].Value, Hunks = new() };
                res.Files.Add(cf);
                ch = null;
                continue;
            }
            if (cf == null) continue;
            var hm = HunkHeader().Match(l);
            if (hm.Success)
            {
                ol = int.Parse(hm.Groups["os"].Value);
                nl = int.Parse(hm.Groups["ns"].Value);
                ch = new() { Header = l, Lines = new() };
                cf.Hunks.Add(ch);
                continue;
            }
            if (ch == null) continue;
